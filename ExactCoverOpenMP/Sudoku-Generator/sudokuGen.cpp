...
public:
  ...
  void loadFile(std::ofstream& FileToLoad);
};
...
void Sudoku::loadFile(std::ofstream& FileToLoad)
{
  for(int i=0;i<9;i++)
  {
    for(int j=0;j<9;j++)
...
{
  int puzzleN = atoi(argv[1]);
  ofstream FileToLoad("puzzles.txt");
  ...
  for(int i=0; i < puzzleN; i++){
    ...
    //loading the file with the current puzzle
    puzzle->loadFile(FileToLoad);
